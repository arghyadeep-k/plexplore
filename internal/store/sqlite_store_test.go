...
func TestSQLiteStore_InsertSpoolBatch_PartialDuplicates(t *testing.T) {
	...
	var lastSeq int
	if err := store.db.QueryRow(`SELECT last_seq_received FROM devices WHERE user_id = 1 AND name = ?;`, "d1").Scan(&lastSeq); err != nil {
...
func TestSQLiteStore_InsertSpoolBatch_MultipleDevices(t *testing.T) {
	...
	for device, want := range map[string]int{"d1": 1, "d2": 3} {
		var got int
		if err := store.db.QueryRow(`SELECT last_seq_received FROM devices WHERE user_id = 1 AND name = ?;`, device).Scan(&got); err != nil {
